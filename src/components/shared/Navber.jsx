      ...
      <AppBar position='static'>
        <Container maxWidth='md' sx={{ background: '#fff' }}>
          <Stack my={2}>
            <Link>
              <img
                src={process.env.REACT_APP_FRONT_URL + '/images/logo.svg'}
                alt='Like It!'
              />
            </Link>
            <Stack sx={{ ml: 'auto', alignItems: 'end' }}>
              <IconButton sx={{ display: breakpoint.mobile }} size='small'>
                <ModalBox openBtn={<SearchIcon fontSize='small' />}>
                  {searchBox}
                </ModalBox>
              </IconButton>
              {isAuthenticated ? (
          ...
          <Stack my={2}>
            <Stack
              ...
              sx={{ mr: breakpoint.margin, cursor: 'pointer' }}
            >
              {categoryOpen ? (
                <MenuOpen fontSize='small' />
              ) : (
                <MenuClose fontSize='small' />
              )}
              &nbsp;
